...
def register():
    ...
    if str(level) not in ALL_LEVELS:
    ...
    c.execute("INSERT INTO payments (user_id, amount, status) VALUES (?, ?, 'unpaid')", (user_id, amount))
    conn.commit()
...
def search_courses():
    ...
    # All levels up to and including the user's own level
    eligible_levels = [str(lvl) for lvl in [100, 200, 300, 400, 500] if lvl <= user_level]
...
def get_user_accessible_courses(user_id):
    ...
    c.execute("""SELECT id, course_code, course_title, description, level, semester
                 FROM courses WHERE TRIM(level)=? AND semester=? ORDER BY id DESC""",
